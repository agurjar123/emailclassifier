...
userId = service.users().getProfile(userId='me').execute()
print(userId)
...
msg_list = service.users().messages().list(userId='me', maxResults = maxRESULTS, pageToken = pageToken, q = q, labelIds = labelIds, includeSpamTrash = includeSpamTrash).execute()
#msg_list['messages'][0]['id']

format = 'raw'
fields = 'labelIds, raw'
emails = []
for i in range(len(msg_list['messages'])):
    email = service.users().messages().get(userId='me', id = msg_list['messages'][0]['id'], format = format, fields = 'labelIds, raw').execute()
    emails.append(email)
print(emails)
